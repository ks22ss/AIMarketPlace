...
- **POST `/api/chat`**: body `message`, optional `skill_id`, optional `conversation_id` (`public-api.ts`). Requires a configured chat model (`CHAT_API_KEY` or `OPENAI_API_KEY` or `MINIMAX_API_KEY`, plus base URL / `LLM_MODEL` — see `chat-llm.ts`). Default response is JSON `{ reply, traceId, conversationId, conversationTitle }` (`traceId` is a UUID per request). Every successful turn is appended to `chat_conversations` — when `conversation_id` is omitted a new conversation is created and titled from the first six words (≤48 chars) of the reply with any `<think>...</think>` block stripped. If the request includes `Accept: text/event-stream`, the same body/auth apply but the response is **SSE**: events `meta` (`trace_id`), zero or more `token` (`delta`), then `conversation` (`{ conversation_id, title }`) and `done` (`{ reply, conversation_id, title }`), or terminal `error` (`message`). Only the **final** prompt node in the skill graph streams tokens; earlier nodes still buffer with `invoke`.
- **Chat history**: `chat_conversations` is a per-user table with a JSONB `messages` array (`{ id, role, content, traceId?, createdAt }`). CRUD endpoints are scoped to the authenticated user:
  - **GET `/api/chat/conversations`** — summaries (newest `updated_at` first).
  - **GET `/api/chat/conversations/:id`** — full conversation with messages.
  - **PATCH `/api/chat/conversations/:id`** — body `{ title }`, rename only (1–120 chars).
  - **DELETE `/api/chat/conversations/:id`** — hard delete. 404 is returned for another user’s id to avoid leaking existence.
- **Execution** (`lib/agent/runtime.ts`): a **LangGraph `StateGraph`** is compiled per distinct workflow shape. `buildSkillExecutionOrder` computes the step list, then `compileSkillGraph` maps each step to a graph node and wires linear `START → step[0] → … → END` edges. `runSkill` reuses a module-level compiled graph cache keyed by **pipeline on/off + resolved step order** (bounded FIFO); `compiled.invoke()` runs per request with fresh `query` / user fields. Prompt nodes load `Node` rows by **`state.orgScope`**, not a compile-time org id, so cached graphs are not pinned to one tenant.
...
Authenticated shell: `/` (Chat), `/chat` (legacy alias for Chat), `/nodes`, `/skills`, `/documents`, `/marketplace`. Auth routes: `/login`, `/register`.
