...
- **POST `/api/chat`**: body `message`, optional `skill_id` (`public-api.ts`). Requires a configured chat model (`CHAT_API_KEY` or `OPENAI_API_KEY` or `MINIMAX_API_KEY`, plus base URL / `LLM_MODEL` — see `chat-llm.ts`). Default response is JSON `{ reply, traceId }` (`traceId` is a UUID per request; not persisted). If the request includes `Accept: text/event-stream`, the same body and auth rules apply but the response is **SSE**: events `meta` (`trace_id`), zero or more `token` (`delta`), then `done` (`reply`) or `error` (`message`). Only the **final** prompt node in the skill graph streams tokens; earlier nodes still buffer with `invoke`.
