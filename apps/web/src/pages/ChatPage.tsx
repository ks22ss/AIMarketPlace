    ...
    <main className="flex min-h-svh flex-col items-center px-4 py-10">
      <div className="flex w-full max-w-2xl flex-col gap-4">
        <div className="flex flex-col gap-1">
          ...
          <h1 className="font-heading text-2xl font-semibold tracking-tight text-foreground">RAG chat</h1>
          <p className="text-sm text-muted-foreground">
            Uses <code className="rounded bg-muted px-1 py-0.5 text-xs">POST /api/chat</code> (LangGraph: retrieve →
            answer). Sign in and ingest documents on{" "}
        ...
        {!authLoading && accessToken ? (
          <Card className="flex flex-col overflow-hidden">
            ...
            <CardContent className="flex max-h-[min(520px,70vh)] flex-col gap-2 overflow-y-auto px-4 py-4">
              {lines.length === 0 ? (
                <p className="text-sm text-muted-foreground">Send a message to test retrieval + the LLM reply.</p>
              ) : null}
              {lines.map((line) => (
                <div
                  key={line.id}
